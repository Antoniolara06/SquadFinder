...
# ─── Token JWT con itsdangerous ────────────────────────────────────────────────

def get_serializer():
    secret = app.config.get('SECRET_KEY', 'squadfinder')
    return URLSafeTimedSerializer(secret)

def generate_token(user_id: int) -> str:
    s = get_serializer()
    return s.dumps({'user_id': user_id}, salt='auth-token')

def verify_token(token: str, max_age: int = 60 * 60 * 24 * 7):  # 7 días
    s = get_serializer()
    try:
...
def get_games():
    ...
    return jsonify([g_obj.to_dict() for g_obj in games]), 200
...
def get_config():
    ...
    return jsonify({
        'juegos': [gm.to_dict() for gm in games],
...
def get_my_anuncios():
    user = g.current_user
    anuncios = Anuncio.query.filter_by(user_id=user.id).order_by(Anuncio.created_at.desc()).all()
    return jsonify([a.to_dict() for a in anuncios]), 200
...
def get_friends():
    user = g.current_user
    friends_1 = Friendship.query.filter_by(user_id=user.id, status='accepted').all()
    friends_2 = Friendship.query.filter_by(friend_id=user.id, status='accepted').all()
    ...
    pending_received = Friendship.query.filter_by(friend_id=user.id, status='pending').all()
    pending_sent = Friendship.query.filter_by(user_id=user.id, status='pending').all()
...
def send_message(friend_id):
    ...
    msg = Message(sender_id=user.id, receiver_id=friend_id, content=content)
    db.session.add(msg)
    db.session.commit()
